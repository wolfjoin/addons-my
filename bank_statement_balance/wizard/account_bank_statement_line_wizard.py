...
class PartnerStatementsReportWizard(models.Model):
    ...
    journal_id = fields.Many2one('account.journal', string=u'账户名称', required=True,
                              help=u'查看本次报表的现金/银行账户名称')
    from_date = fields.Date(string=u'开始日期', required=True, 
                            help=u'查看本次报表的开始日期')  # 默认公司启用日期
    						# default=_get_company_start_date, 没法获取
    ...
    def confirm_bank_statements(self):
        ...
        view = self.env.ref('bank_statement_balance.bank_statements_report_tree')

        return {
            'name': u'现金银行报表:' + self.journal_id.name,
            'view_type': 'form',
            'view_mode': 'tree',
